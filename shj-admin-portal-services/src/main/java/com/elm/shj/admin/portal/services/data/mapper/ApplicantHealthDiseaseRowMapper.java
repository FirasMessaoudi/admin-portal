...
/**
 * Mapper that creates an applicant disease data object out of the given excel row data
 *
 * @author Aymen DHAOUI
 * @since 1.0.0
 */
@Component
public class ApplicantHealthDiseaseRowMapper extends AbstractRowMapper<ApplicantHealthDiseaseDto> {

    /**
     * {@inheritDoc}
     */
    @Override
    public ApplicantHealthDiseaseDto mapRow(Row row) {
        ApplicantHealthDiseaseDto applicantHealthDiseaseDto = new ApplicantHealthDiseaseDto();
        // applicantHealthDiseaseDto.setRowNumber(row.getRowNum());
        ...
        applicantHealthDiseaseDto.setLabelAr(row.getCell(row.getFirstCellNum() + 4).getStringCellValue());
        applicantHealthDiseaseDto.setLabelEn(row.getCell(row.getFirstCellNum() + 5).getStringCellValue());

        applicantHealthDiseaseDto.setCreationDate(new Date());
        return applicantHealthDiseaseDto;
    }
    ...
    public EDataSegment getDataSegmentId() {
        return EDataSegment.APPLICANT_DISEASE_DATA;
    }
}
